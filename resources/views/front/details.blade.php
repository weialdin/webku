				...
				<a id="Author" href="{{ route('front.author', $articleNews->author->slug) }}" class="w-fit h-fit">
					<div class="flex items-center gap-3">
						<div class="w-10 h-10 rounded-full overflow-hidden">
							<img src="{{ Storage::url($articleNews->avatar) }}" class="object-cover w-full h-full" alt="avatar">
						</div>
				...
				<a href="{{ route('front.details', $item_news->slug) }}" class="card-from-author">
					<div
						...
						<div class="flex flex-col gap-[6px]">
							...
							<p class="text-xs leading-[18px] text-[#A3A6AE]">{{ $item_news->created_at->format('M d, Y') }} . {{ $item_news->category->}}</p>
						</div>
					</div>
				</a>
				@empty
